...
class EnhancedQAAnalyticsApp:
    ...
    def _update_results_display(self, processor_results):
        ...
        # Summary tab with side-by-side layout
        summary_frame = ttk.Frame(summary_tab)
        summary_frame.pack(fill=tk.BOTH, expand=True, padx=10, pady=10)

        # Configure the summary frame columns to create a side-by-side layout
        summary_frame.columnconfigure(0, weight=1)  # Left side
        summary_frame.columnconfigure(1, weight=1)  # Right side

        # Statistics on the left
        stats_frame = ttk.LabelFrame(summary_frame, text="Results Summary")
        stats_frame.grid(row=0, column=0, sticky=(tk.N, tk.W, tk.E, tk.S), padx=(0, 5))
        ...
        # Group results if available - now placed to the right
        summary_data = processor_results.get('summary')
        if summary_data is not None:
            # Group results table - now placed on the right side
            group_frame = ttk.LabelFrame(summary_frame, text="Group Summary")
            group_frame.grid(row=0, column=1, sticky=(tk.N, tk.W, tk.E, tk.S), padx=(5, 0))
            ...
            # Pack tree and scrollbar
            group_tree.pack(side=tk.LEFT, fill=tk.BOTH, expand=True, padx=10, pady=10)
        ...
        if detail_data is not None:
            ...
            # Fixed filter function that properly resets the view
            def apply_filter():
                ...
                if not detail_tree["columns"]:
                    detail_tree["columns"] = list(detail_data.columns)
                ...
                # Filter the data
                if filter_value == "gc":
                    filtered_df = detail_data[detail_data["Compliance"] == "GC"]
                elif filter_value == "dnc":
                    filtered_df = detail_data[detail_data["Compliance"] == "DNC"]
                else:
                    filtered_df = detail_data
